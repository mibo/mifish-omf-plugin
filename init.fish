...
abbr -a git_mb_rm_gone_branch_dry "git branch -v | grep gone | cut -f 3 -d ' '"
abbr -a git_mb_rm_gone_branch "git branch -D (git branch -v | grep gone | cut -f 3 -d ' ')"
# SAP CP CF
abbr -a cf_canary_config "cp ~/.cf/config.canary.json ~/.cf/config.json"
abbr -a cf_live_config "cp ~/.cf/config.live.json ~/.cf/config.json"
#
